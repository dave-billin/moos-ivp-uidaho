...
bool iSpock::OnStartUp( void )
{
    ...
    if ( 0 != m_SpockUDPPort )
    {
       MOOSTrace( "Sensors will be obtained from received UDP sensor packets\n");
    }
    ...
    try
    {
       MOOSTrace( "Opening a TCP connection to a SPOCK at " + m_SpockHostname
                  + MOOSFormat(":%d\n", m_SpockTCPPort) );
        m_pSpock = new SpockModule( m_SpockHostname, m_SpockTCPPort,
                                    m_SpockUDPPort, m_Verbosity );
    }
...
bool iSpock::LoadMissionFileParameters( void )
{
    ...
    if ( !m_MissionReader.GetConfigurationParam("SPOCK_TCP_PORT", m_SpockTCPPort) )
    {
        return MOOSFail(szParamFail, s.c_str());
    }
   ...
   // Get the network port to connect to on the SPOCK module
   m_MissionReader.GetConfigurationParam("SPOCK_UDP_PORT", m_SpockUDPPort);
