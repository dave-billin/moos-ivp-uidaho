...
public:
	...
	inline float CompassHeading(void) const { return m_CompassHeading; }

	/** Returns the compass pitch angle in radians */
	inline float CompassPitch(void) const { return m_CompassPitch; }

	/** Returns the compass roll angle in radians */
	inline float CompassRoll(void) const { return m_CompassRoll; }

	/** Returns the compass dip angle in radians */
	inline float CompassDip(void) const { return m_CompassDip; }

	/** Returns the measured depth in meters based SPOCK pressure sensor */
	inline float Depth(void) const { return m_Depth; }

	/** Returns the measured pitch from the SPOCK accelerometers in radians */
	inline float AccelPitch(void) const { return m_AccelPitch; }

	/** Returns the measured roll from the SPOCK accelerometers in radians */
	inline float AccelRoll(void) const { return m_AccelRoll; }


	/** Returns the GPS latitude in degrees */
	inline float GpsLatitude(void) const { return m_GPS_Latitude; }

	/** Returns the GPS longitude in degrees */
	inline float GpsLongitude(void) const { return m_GPS_Longitude; }

	/** Returns the GPS horizontal position error in meters */
	inline float GpsHPE(void) const { return m_GPS_HPE; }

	/** Returns the GPS velocity in meters per second */
	inline float GpsVelocity(void) const { return m_GPS_Velocity; }

	/** Returns the GPS heading in degrees (adheres to MOOS conventions */
	inline float GpsHeading(void) const { return m_GPS_Heading; }

	/** Returns the GPS hours in the day */
	inline uint32_t GpsHours(void) const { return m_GPS_Hours; }

	/** Returns the GPS minutes in the hour */
	inline uint32_t GpsMinutes(void) const { return m_GPS_Minutes; }

	/** Returns the GPS seconds in the minute */
	inline uint32_t GpsSeconds(void) const { return m_GPS_Seconds; }

	/** Returns true if a water leak is detected */
	inline bool WaterLeakIsDetected(void) const {return m_WaterLeakIsDetected;}

	/** Returns the temperature on the SPOCK gyros in degrees C */
	inline float Temperature(void) const { return m_Temperature; }

	/** Returns the measured AUV battery bus voltage in DC volts */
	inline float BatteryVoltage(void) const { return m_BatteryVoltage; }
