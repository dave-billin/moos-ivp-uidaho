...
struct bsockKeepAlivePacket_t
{
	...
	char Unused[BUNNYSOCK_PACKET_SIZE -
	            sizeof(bsock_PacketHeader_t) -
...
{
	BunnySockTcpNode* pObjectInstance = (BunnySockTcpNode*)pObject;
...
: BunnySockNode(NULL, 0, 0),
  m_sPeerHostname("0.0.0.0"),
  m_Port(0),
  m_ConnectionMode(-1),
  m_RetryPeriodSec(2000),
  m_ConnectionTimeout_ms(BUNNYSOCK_DEFAULT_CONNECTION_TIMEOUT_MS),
  m_PeerDeviceId(0),
  m_TxLock(false),
  m_pSocket(NULL),
  m_WorkerThread()
  ...
  m_WorkerThread()
{
	...
	pKeepAlivePacketData = (bsockKeepAlivePacket_t*)m_KeepAlivePacket.GetRawBytes();
...
bool BunnySockTcpNode::SendPacket( BunnySockPacket& PacketToSend )
{
	...
	if (m_IsConnected)
	{
		...
		if ( m_TxLock.Lock(100) )
		{
			try
			{
				m_pSocket->iSendMessage(PacketToSend.GetRawBytes(), BUNNYSOCK_PACKET_SIZE);
			}
			catch (XPCException& e)
			{
				BSOCK_VERBOSE1( MOOSTrace( sBunnySockTcpNode + string(" failed to send packet: ") +
										   e.sGetException() + "\n") );
				PacketWasSent = false;
			}
		...
		{
			BSOCK_VERBOSE1( MOOSTrace(sBunnySockTcpNode + " Timed out on m_TxLock while attempting to "
									  "send a packet!\n") );
			PacketWasSent = false;
		}
	}
...
bool BunnySockTcpNode::OpenClientConnection( void )
{
	...
	while ( !m_WorkerThread.IsQuitRequested() )
	{
		...
		catch (XPCException& e)
		{
			...
			BSOCK_VERBOSE3( MOOSTrace( sBunnySockTcpNode + " Failed to connect to " +
									   m_sPeerHostname + ":\n  " + e.sGetException() + "\n") );
...
bool BunnySockTcpNode::OpenServerConnection( void )
{
	...
	while ( !m_WorkerThread.IsQuitRequested() )
	{
		...
		catch (XPCException& e)
		{
			...
			BSOCK_VERBOSE1( sError = sBunnySockTcpNode +
...
void BunnySockTcpNode::WorkerThreadBody( void )
{
	...
	memcpy(KeepAlivePacket.GetRawBytes(), &KeepAlivePacketData, BUNNYSOCK_PACKET_SIZE);
	...
	while ( !m_WorkerThread.IsQuitRequested() )
	{
		...
		{
			try { RefTime.GetSystemTime(true); }    // Grab current time
			...
			Elapsed_ms = m_LastRxTime.ElapsedTime().As(PrecisionTimeInterval::MILLISECONDS);
			...
			Elapsed_ms = m_LastTxTime.ElapsedTime().As(PrecisionTimeInterval::MILLISECONDS);
			if ( Elapsed_ms > SendThreshold_ms )
			{
				PacketWasSent = SendPacket(KeepAlivePacket);

				try { RefTime.GetSystemTime(true); }
				catch (std::exception& e)
				{
					MOOSTrace("%s %s\n", sBunnySockTcpNode.c_str(), e.what() );
					break;
				...
				if (m_Verbosity >= 4)
				{
					const char* szStatus = ((PacketWasSent) ?
												"Sent" : "Failed to send");
			...
			//--------------------------------------------------
			Elapsed_ms = m_LastTxTime.ElapsedTime().As(PrecisionTimeInterval::MILLISECONDS);
			uint32_t MsToBlock = (Elapsed_ms < SendThreshold_ms) ?
			                            (SendThreshold_ms - Elapsed_ms) : 0;
			...
			if (Flags < 0)
			{
				...
				BSOCK_VERBOSE2( MOOSTrace(sBunnySockTcpNode +
			...
			if (Flags & FileSelector::Readable)
			{
				...
				{
					BSOCK_VERBOSE1( MOOSTrace( sBunnySockTcpNode +
				...
				if (NumBytesSoFar == BUNNYSOCK_PACKET_SIZE)
				{
					...
					// peer's device ID
					if (RxPacket.GetHeader()->PacketType == PACKETTYPE_BUNNYSOCK_KEEPALIVE)
					{
						m_PeerDeviceId = RxPacket.GetHeader()->SourceId;
						BSOCK_VERBOSE4( MOOSTrace( sBunnySockTcpNode +
										MOOSFormat(" Received a keep-alive packet from "
												   "device %d\n", m_PeerDeviceId)) );
					}
					else
					{
